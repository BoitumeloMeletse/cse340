...
async function addVehicleByClassificationName(classification_name) {
  try {
    ...
    const data = await pool.query(`INSERT INTO public.classification (classification_name) VALUES ($1) RETURNING *`, [
      classification_name,
    ])
    return data.rows
  } catch (error) {
...
async function addNewInventoryVehicle(
  classification_id,
  inv_make,
  inv_model,
  inv_description,
...
module.exports = {
  ...
  getInventoryById,
  addVehicleByClassificationName,
  addNewInventoryVehicle,
  updateInventory,
  deleteInventory,
}
